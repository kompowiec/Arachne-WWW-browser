...
//p:               *npalout > npalin) return 2; //test pripustnosti
  ...
  ireturn = 4;
  IR= farmalloc( (unsigned long)(2 *npalin) ); //pomocna pole
  if(IR == NULL) return 4;
  IG= farmalloc( (unsigned long)(2 *npalin) ); //pomocna pole
  if(IG == NULL) goto freeIR;
  IB= farmalloc( (unsigned long)(2 *npalin) ); //pomocna pole
